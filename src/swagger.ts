...
// export default function (fastify, opts, next): string {
...
//             return next(new Error(`${opts.specification.path} does not exist`))

//         const extName = path.extname(opts.specification.path).toLowerCase()
//         if (['.yaml', '.json'].indexOf(extName) === -1)
//             return next(
//                 new Error('specification.path extension name is not supported, should be one from [.yaml, .json]'),
//             )

//         if (opts.specification.postProcessor && typeof opts.specification.postProcessor !== 'function')
//             return next(new Error('specification.postProcessor should be a function'))

//         if (opts.specification.baseDir && typeof opts.specification.baseDir !== 'string')
//             return next(new Error('specification.baseDir should be string'))
...
//             return next(new Error('specification.document is not an object'))

//         swaggerObject = opts.specification.document
//     }

//     fastify.decorate('swagger', fastify.swagger)
...
//         fastify.register(import('./routes'), options)
//     }
